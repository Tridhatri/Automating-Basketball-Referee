...
class CameraLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.step_count_label = self.ids['step_count_label']
        self.update_step_count()

    def update_step_count(self):
        self.step_count_label.text = f'Step Count: {step_count}'

    def update(self, dt):
        global step_count, prev_left_ankle_y, prev_right_ankle_y, wait_frames
...
class CameraApp(App):
    def build(self):
        self.layout = CameraLayout()
        self.cap = cv2.VideoCapture(0)
        Clock.schedule_interval(self.layout.update, 1.0 / 30.0)
        return self.layout
